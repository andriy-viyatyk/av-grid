# av-grid — Implementation Plan 12 (done): phase 16, a filter or a sort on a `hidden` column survives the column being hidden

**Phase 16 — done, shipped as 2.9.2 on 2026-09-07.** Phases 1–15 shipped through **2.9.1**. One task,
from a consumer report on 2.9.x (2026-09-07). **Before starting anything later, read the decision logs
in `plan-done-01.md` … `plan-done-11.md`** — every one of them still applies. Task 57 was a
**defect**, not a refinement — no prior decision stood behind it. The decisions the fix itself made
are logged below; the first of them — **identity resolves over the full column set, geometry over the
visible one** — is the rule later work has to keep.
...
The open questions this plan carried (the popovers' control-size tokens; a grid-level
`textFilterOps` default; `textFilterLabels`; a runtime setter that degrades instead of throwing,
raised with task 57) move forward unchanged to [`plan.md`](plan.md). None is committed.
