...
**Archived — all six tasks are ✅.** The task-by-task build order for phase 6. [`goal.md`](goal.md)
...
| 2026-08-11 | fix | **A percentage column width fits on its own, so it suppresses the trailing whitespace too — a divergence from the reference.** `calcInnerSize` adds `whiteSpace` (20 px) of slack past the last column so the final one can scroll clear of the edge, and drops it under `fitToWidth` because the columns already total the width. But `buildLengthArray` runs the same fit whenever *any* width is a percentage, `fitToWidth` or not — so a grid with one `width: "35%"` column got a perfect fit **and** 20 px of slack on top of it, which is a horizontal scrollbar by construction. Measured on `CustomizationBoard`: root 1215, scrollbar 12, columns fitted to exactly 1203, `area` 1223. The vertical scrollbar was never the problem — `size.width - scrollBarWidth` had it right all along. `hasPercentLength()` now computes `columnsFitted`, which `calcInnerSize` and `rerender-check`'s resize branch read in place of the option. The reference carries the same hole and never falls in it, because Persephone only ever writes a percentage alongside `fitToWidth` — `FileGrid.tsx:100` says so in a comment. Costs one pass over the **columns** (tens, and never the rows, which cannot be percentages); the 100k gate is unmoved at 1.03×, 60/60 fps. |
