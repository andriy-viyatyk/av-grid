...
| 2026-09-03 | 54–55 | 1.7 ms | 0.84× | 60.0 / 60.0 | 0 | **`blank` / `notBlank` on the `"text"` filter (`textFilterOps`), and `filterLabel`.** Not a render-path change: a second arm in `filtersMatch`, the operator table moved to `src/textFilterOps.ts`, the popover body reading `Column.textFilterOps`, and the bar building its chips from `describeFilter` (one resolver, which is where the host `filterLabel` runs). Gate carried from the task-53 row — no file under `src/render/` or the cell renderers changed. The per-pass cost at 100,000 rows on the CustomizationBoard's Owner column (a third of the values null or `""`), three alternated rounds each: **`blank` 2.2 / 3.1 / 3.1 ms**, **`notBlank` 2.4 / 3.1 / 2.9 ms**, against **`contains` 7.6 / 4.7 / 4.7 ms** and **`equals` 3.4 / 3.6 / 4.1 ms** on the same column — the state operators at or under the comparing ones, as expected: no needle, one `String().trim()` per row. Board: `checkAll` **35/35** (+6: blank and notBlank row counts against the displayed-text rule, the default column still showing three chips, the five-chip popover with the applied op pressed, the chip reading `is not empty`, and `filterLabel` renaming one chip with `describeFilter` agreeing). Live finding: the popover does not wrap five chips — it has an intrinsic width, so it widens to ~354 px on one row, which reads better than two rows; `flex-wrap` stays as the safety net. The example page's fake server translated `{ op: "blank" }` to an IS NULL and the chip read the page's `filterLabel` word. |
| 2026-09-16 | 63 | 9.1 ms | 0.79× | 60.0 / 60.0 | 0 | **Task 63, shipped as 2.11.3** — every popover now mounts into the nearest open `<dialog>` above its anchor instead of always on `document.body`, because a modal dialog is in the browser's top layer and makes the body inert. **No render-path file changed**: the diff is `Popover`, `Menu`, `ContextMenu` and `FilterPopover`, none of which a cell paint touches, and one `getAttribute`-free `closest()` runs once per popover *open* — never per frame. The row is here because the standing rule is to measure rather than assume, and the gate is unmoved: first paint 9.1 ms, flat 0.79×, 60.0 / 60.0 fps, full repaint 0.10 ms / 0 mutations, drag 0 mutations over 200 steps, select-all 23.7 ms. |
| 2026-09-21 | 64 | 9.0 ms | 0.89× | 60.0 / 60.0 | 0 | **Task 64, shipped as 2.12.0** — `treeColumn.busy`: the chevron replaced by a spinner in its own slot while a node's children load. `DataCell` *is* a render-path file, so this was measured rather than assumed, and the first measurement paid for itself: the tree gutter went from **1.01× to 1.13×** per scroll frame, because the spinner factory was passed down as a bound thunk — one closure allocated per tree cell per paint, for a call the slot makes only when it enters the busy state. Passing the options and the row instead put it back to **1.00×** (0.143 against 0.143 ms at the top, 0.127 against 0.129 at the bottom, 0 gutter mutations on a full repaint, all 14 tree checks green). The 100k gate on the final code: first paint 9.0 ms, flat 0.89×, 60.0 / 60.0 fps, full repaint 0.0 ms / 0 mutations, drag 0 mutations over 200 steps, select-all 21.7 ms, sort 21.0 ms. |
| 2026-09-23 | 65 | 3.5–4 ms | 0.88× | 60.0 / 60.0 | 0 | **Task 65, prototype on a branch — not shipped.** `RenderGrid.eagerScrollPaint`: when a scroll leaves the band the last paint covered, paint *in the scroll handler* instead of waiting for that frame's animation callbacks. Measured against itself with the switch, six alternating 100k gate runs: flat ratio medians **1.06× off / 0.88× on** (off 0.99 / 1.16 / 1.06, on 0.83 / 0.88 / 1.05 — overlapping, so no measurable cost), 60 / 60 fps and 0 pool misses either way. Per-frame A/B at the two distances where the path actually fires (`syncPaints` 60/60 and 30/30): **identical DOM work** — same paints, same cells appended and removed, 11,400 and 25,404 mutations on both sides — with paint time 0.398 → 0.413 ms on a quarter-viewport step and 1.70 → 2.17 ms on a scrollbar drag, and pool misses on the drag **72 → 0**. What it buys, probed per frame under a *browser-driven* scroll: blank frames **33 % → 0 %** at a quarter viewport and **100 % → 0 %** on a drag; on the comparison board, the same rows go 25 % → 0 % and 100 % → 0 %. **A programmatic per-frame `scrollTop` write cannot see any of this** — its event is delivered after the frame it describes, so every grid measures blank; both probes here were rewritten to drive the browser instead. |
| 2026-09-24 | 65 | 1.6 ms | 0.93× | 60.0 / 60.0 | 0 | **Task 65, prototype on a branch — `RenderGrid.ownScroll`.** The container keeps its native scrolling (so the wheel and the trackpad stay on the compositor with the browser's own animation) and only its scrollbar is hidden; the bar the user drags is an **empty strip**, whose scroll event writes the offset, recomputes and paints in one task. Six alternating gate runs, medians, 36 visible rows on both sides: first paint **1.6 ms against 1.7**, paint **0.210 / 0.195 ms against 0.207 / 0.193**, flat ratio **0.93× against 0.94×**, 60.0 / 60.0 fps, 0 pool misses. **Two earlier readings of this row were wrong and are worth the warning.** The first compared the mode against itself: the benchmark grid sets `growToHeight`, which an early revision used to disable the mode, so the flag changed nothing on either side — always assert the thing under test is actually on (this row's run checks for a strip in the DOM). The second was read across runs of different sizes, because the results table grows above the grid and shrinks its viewport after run 1; the board now hides the table while measuring, and every run reports the same 36 rows. With both fixed the mode measured **five times the paint cost** — 0.997 / 0.98 / 1.265 ms against 0.223 / 0.203 / 0.165 — at an unmoved 60 fps, which is exactly the kind of regression only this board can see. The cause: putting the thumbs back calls `scrollTop`, which forces a synchronous layout, and it ran inside the paint, which had just dirtied two million-pixel-tall boxes (the cell area and the strip's spacer). Deferring that write to the next frame, where layout is clean, restored the numbers above; the thumb trails the rows by one frame, which no eye can see. |
| 2026-09-14 | 62 | 13.1 ms | 0.90× | 60.0 / 60.0 | 0 | **Task 62, shipped as 2.11.2** — a data, footer or tree cell recycled from a header cell no longer keeps the header's *children*. Render-path change: `claim(el, kind)` at the top of `renderDataCell` / `renderFooterCell` reads `data-type` **before** the renderer overwrites it and, when the kind changed, drops the element's `mode` / `written` / `treeState` records; `setMode` lost its `data-type` compare (moved, not added — one `getAttribute` per paint either way). The 100k gate is unmoved: full repaint 0.10 ms / 0 mutations, drag 0 mutations, scroll paint 0.15–0.17 ms/frame, first paint 13.1 ms (12.8 at task 61). The consumer's repro on AVGridBoard — 60 columns × 5,000 rows, every column's `render` returning the same string, sorted on the right-most column, three sweeps right/left/down/up, 17,320 pool hits: **pre-fix 70 of 275 data cells held the header's children at rest (83 at the peak)**; fixed **0 / 0**, and 0 cells with a wrong text. **Second finding, fixed by the same change:** the tree content host has no `data-type`, so the old `setMode` compare emptied and rebuilt it on every paint — a `refresh()` over 16 tree cells did **48 child-list node mutations before, 0 after** (attribute records 301 → 253, the difference being exactly those). |
| 2026-09-11 | 61 | 12.8 ms | 0.94× | 60.0 / 60.0 | 0 | **Task 61, shipped as 2.11.1** — a recycled header cell no longer leaves its `title`, `aria-sort` or `draggable` on a data or footer cell. Render-path change: `clearForeignHeaderState(el)` in `renderDataCell` and `renderFooterCell` — two `removeAttribute` calls on attributes that are usually absent plus a `draggable` read. The 100k gate is unmoved: full repaint 0.10 ms / 0 mutations, drag 0 mutations, scroll paint 0.20–0.22 ms/frame, first paint 12.8 ms (12.4 at task 58–59; 9.9 at task 60 — run-to-run noise). The consumer's repro on AVGridBoard, 60 columns × 20k rows sorted on the right-most column, swept right/left/down until the pool had served 8,420 reuses: **pre-fix 38 of 574 data cells carried a stale header `title` and were `draggable`**; fixed build **0 / 0 / 0**, the 14 header cells keep their own titles. `aria-sort` did not leak in that sweep (the sorted header was not evicted) — the unit test covers it through the pool. |
| 2026-09-09 | 60 | 9.9 ms | 0.90× | 59.9 / 59.9 | 0 | **`headerHeight`, shipped as 2.11.0** after the author verified the board. A render-path change only in the sense that `engineRowHeight()` now hands the engine a per-row function whenever the header differs from the rows — the same function shape column groups have used since 2.6.0; `HeaderCell` and `GroupHeader` read `model.headerBand()` instead of `rowHeight`. The plain 100k gate is unmoved: full repaint 0.10 ms / 0 mutations, drag 0 mutations. `measureHeaderHeight()` on AVGridBoard, 100k rows at 56px with a three-line `render`, header 26: first paint 6.4 ms; flat header 26.00 px over 56.00 px rows; grouped band 26.00, leaf header 26.00, ungrouped header 52.00, **band-bottom to leaf-top seam 0.00 px**; the option removed live puts the grouped header back on 112 (2 × 56); `getState().headerHeight` reports the band; 9/9 checks. Scroll paint per frame with the function against the plain number, two runs each: 0.088–0.094 ms both ways (ratios 0.94 / 1.07 — frame noise at a 0.09 ms paint); full repaint mutations identical with and without. Live from the toolbar: header 26 → cleared 56 → 30, each on the first read after `settle`. Screenshot: the consumer's shape — a 52 px two-row header over 56 px three-line rows. |
| 2026-09-08 | 58–59 | 12.4 ms | 0.79× | 60.0 / 60.0 | 0 | **`disableColumnReorder` and `treeColumn`, shipped together as 2.10.0** after the author verified the boards. The 100k gate on the plain grid is unmoved: full repaint 0.10 ms / 0 mutations. Task 58 is not a render-path change (one predicate, three readers); verified live — the *Reorder off* checkbox takes `draggable` off all ten data headers, a `dragstart` is cancelled, the resize grip stays, unchecking restores them. Task 59 **is** a render-path change (`renderDataCell` gains a tree branch that writes into a content host): `measureTree()` on AVGridBoard, 10,520 tree rows fully expanded, a pinned-left tree column with the host's `render` (icon + label + action button) beside grouped columns — first paint 8.1 ms, scroll paint **0.121 ms at the top against 0.121 ms with `treeColumn` removed (1.01×)**, 0.145 ms near the bottom, **0 mutation records inside a row's gutter across a full repaint**, a chevron press through a real `pointerdown` **0.9 ms** including the host's re-flatten (10,520 → 9,995 rows), all 14 checks pass (shapes at three depths, `aria-expanded`, path copied as `North › Aetna › Members`, press moves no focus and is not a cell click, `→` re-expands, `→` on a leaf navigates, gutter gone when the option comes off, static chevron inert, roots-always-open shape). Three runs of the gate: gutter cost ratio 1.07 / 1.18 / 1.01 — within the frame-to-frame noise of a 0.12 ms paint. Screenshots: the collapsible tree with a focused leaf showing the host's action button; the static roots-open tree. |
| 2026-09-07 | 57 | 14.0 ms | — | 60.0 / — | 0 | **A filter or a sort on a `hidden` column survives the column being hidden.** Not a render-path change: `FiltersModel`, `setSort`, `SortColumnModel` and `describeFilter` now resolve a filter's or a sort's column over the *full* column set (`ColumnsModel.columnByKey`), and `filterRows` gained an optional trailing `filterColumns` parameter (default: `columns`) so `RowsModel` passes the visible set for the search and the full set for the filters. Re-measured anyway because the filter pass changed signature: `measureFilter(100000)` on AVGridBoard, three runs — filter down **7.3 / 8.8 / 4.1 ms**, narrow **6.2 / 8.8 / 3.3 ms**, back up **0.2 / 0.3 / 0.2 ms**, **1 paint** each, 60.0 fps filtered — inside the task-25 row's spread (3–10.6 ms cold to warm); the lookup is once per filter per pass, never per row. Live gesture verified: filter Team = platform, sort by Team, hide Team through `setOptions({ columns, filters, sort })`, then `setFilters` with the same list and `setSort` on the hidden column — no throw, 20,000 of 100,000 rows kept, the bar chip still reads `Team: platform`, the footer `sort: team asc`, Team's header gone (screenshot). |
