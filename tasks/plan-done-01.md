# av-grid — Implementation Plan 01: the port (tasks 1–20) — ✅ complete

**This plan is finished and archived.** All twenty tasks are done; the current work is in
[`plan.md`](plan.md). Nothing here is to be edited — it is the record of how the port was
built, and its **decision log at the bottom is still live reading**: it holds every place the
port deliberately diverges from the reference, including the reference bugs that were fixed
rather than carried over. Read it before starting any task in the current plan.

The task-by-task build order. [`goal.md`](goal.md) is the *what* and *why*; this document was
...
| 19 | `docs/api.md` | Docs | ✅ Done |
| 20 | `examples/` | Docs | ✅ Done |
...
>
> **Undeferred in the polish phase (2026-08-11).** `ContextMenuModel.tsx` *is* ported now, as
> `view/ContextMenu.ts` on a new `view/Menu.ts`, and the host hook shipped with it as
> `onGridContextMenu(e, items)` — see the decision log.
...
| 2026-08-11 | polish | **The context menu is no longer deferred.** `goal.md` scoped it out of v1 — "ship an `onContextMenu` callback and let the host draw its own menu" — because `ContextMenuModel.tsx`'s one hard dependency was Persephone's app shell, `showAppPopupMenu`. Once `Popover` existed, the menu that function reached for was a hundred lines of DOM, so the port is now the whole file: the item list, the labels that count what the selection covers, and the rule that an open editor keeps the platform's own menu. The host hook shipped as well — `onGridContextMenu(e, items)` — so both halves of the scope note are satisfied rather than one instead of the other. |
| 2026-08-11 | polish | **`Menu` is plain DOM, not a `VirtualList`.** A menu is tens of actions at the outside, and no two rows are alike — icon, hotkey, check, submenu chevron. Pooling elements whose content is not uniform buys nothing and costs the ability to write a row once and leave it. The search box above twenty items is the reference's threshold and is kept: past that, reading is slower than typing. |
| 2026-08-11 | polish | **It is a function, not a model.** The reference's `ContextMenuModel` held no state — it subscribed to one event and built an array — and the menu it opens is view code, which the model layer deliberately does not import. So `gridContextMenuItems(model, event)` is a pure function of the grid, which is also what makes "what does the menu offer" testable without opening one, and `flags.contextMenu` is the same structural handle `flags.filterPopover` already was. |
| 2026-08-11 | polish | **`e.selection` is a getter, and that is the whole performance story.** The reference called `getGridSelection()` up front; here that slices the displayed rows, so a right-click with all 100,000 rows selected would copy 100,000 references before drawing a label. Every built-in item needs only `selectedCount`, which is four integers. Measured: opening the menu costs **2.3 ms with 100,000 rows selected against 2.9 ms with one**, and **0 dirty marks and 0 DOM mutations** either way. |
| 2026-08-11 | polish | **Insert is never greyed out.** The reference disabled it on a sorted or filtered grid and put `Filtered or Sorted` where the shortcut goes. Here `addRows` freezes the row order instead, so the row lands where the user put it and the item simply works — the same divergence task 14 already made for ctrl+Insert. |
| 2026-08-11 | polish | **`onCellContextMenu` fires whatever happens next.** It is a notification, not a veto, so it runs before the menu decision and also when `disableContextMenu` hands the gesture to the browser. A host listening for right-clicks should hear all of them, not only the ones the grid drew a menu for. |
| 2026-08-11 | polish | **Right-click inside a selection was already correct.** `FocusModel`'s `rightClick` selection type keeps a range the pointer is inside and re-anchors when it is outside, so "Delete 3 rows" always means the three rows the user can see highlighted. Nothing had to be added for the menu; the type existed and had no caller that exercised it. |
| 2026-08-11 | polish | **A menu row activates on `click`, against the invariant that governs cells.** Nothing repaints a menu underneath the pointer — its rows are written once and left alone — so the identity problem that forces the boolean checkbox onto `pointerdown` does not exist here. The opposite risk does: a press-activated row would fire on the same press that opened the menu when a menu is opened from a `pointerdown` handler. |
| 2026-08-11 | polish | **The chrome tokens now name their `--p-*` counterparts instead of deriving them.** `--avg-header-bg` was `color-mix(text 7%, bg)` — a *lighter* tint, which is right on a white page and backwards in every dark theme, where Persephone's own grid header is `--color-bg-dark` (#181818) against a #1f1f1f body. It is `var(--p-bg-dark, <the old mix>)`, so a board gets the app's chrome surface and a bare HTML page gets what it always got. The filter bar takes the same pair, because it is the same band of chrome. |
| 2026-08-11 | polish | **Cell lines are a separate token from borders, at half the contrast.** `--avg-border-color` (text at 22%) drew the grid lines *and* the popover edges, so making the lines recede would have thinned every structural border with them. `--avg-grid-line` splits them: `var(--p-border-light, color-mix(text 11%, transparent))`, which resolves to Persephone's `--color-grid-border` #2b2b2b exactly. The reference keeps the same two colours apart for the same reason — `grid.borderColor` #2b2b2b against `border.default` #3c3c3c. The resize grip stays on `--avg-border-color`: it is an affordance the pointer is hunting for, not texture. |
| 2026-08-11 | polish | **The accent is for state the grid is reporting, not for anything under the pointer.** The add-row and add-column buttons turned blue on hover, which reads as "this is filtered/sorted/selected" rather than "this is hoverable"; they now brighten to `--p-text-strong` (#dddddd from #969696), as the reference's icon buttons do. |
| 2026-08-11 | polish | **A menu row takes the full selection colour; a checklist row keeps the tint.** Both were `--avg-selection-bg` (accent at 18%), which is correct for `VirtualList`, where several rows are on at once and the labels under them have to stay readable. Exactly one menu row is ever lit, and the platform paints that one solid — `--p-selection-bg` / `--p-selection-text`, #0078d4 on #ffffff, which is what `menu.selectionBackground` is in the theme definitions. The hotkey, icon, chevron and check mark follow the label onto it: left muted they are illegible on the accent, and the check, drawn *in* the accent, vanished into it. |
| 2026-08-11 | polish | **A menu row is 26px, the reference's `ROW_HEIGHT`, and the number lives in two places.** It was 24. The CSS owns the height and `MENU_ROW_HEIGHT` in `Menu.ts` only counts rows for PageDown/PageUp, so the two drifting apart breaks nothing visible — it just makes the key jump by the wrong amount, which is why both carry a comment pointing at the other. |
| 2026-08-11 | polish | **A click into a focused cell puts the caret where it landed; only Enter / F2 / `startEdit()` select the value.** The port selected on every open, so reaching for a cell with the mouse to fix one character armed the whole value for deletion — the opposite of what a click means anywhere else on the platform. The reference already had the split (its `onCellMouseDown` passes `dontSelect: true`, Enter/F2 pass `false`); what it did not have is the position, which is the ask this closes: someone who wanted to replace the value would have typed over the cell instead of clicking it. |
| 2026-08-11 | polish | **The clicked position is measured by real layout, not canvas metrics.** A hidden `<span>` with the input's own computed font, a `Range` over its text node, and a binary search over prefix widths rounding to the nearer boundary — so kerning, ligatures and `letter-spacing` are right because it *is* the shaping the input will do. `caretPositionFromPoint` was not used: over an `<input>` the node it returns is engine-dependent. It runs once per editor open, never in the paint path. |
| 2026-08-11 | polish | **⚠ It returns `undefined` rather than a number when there is no layout.** happy-dom, or a `display: none` ancestor: every rect is zero, so a search would confidently answer **0** for any x and every click would land the caret at the start of the text. Zero total width is the signal, and the caller falls back to the end — the same place a press that carried no coordinate goes. A right-aligned column needs the origin computed from `right − textWidth`, or every click in a numeric cell reads as past the end. |
| 2026-08-11 | polish | **A date or number cell maps the pixel against the raw value, not the formatted one.** The cell displays `1/1/2020, 3:00:00 AM` and the editor holds `String(theDate)`, so the index is clamped to the value's length: approximately right, never worse than end-of-text. Formatting-aware reverse mapping is not possible at all once a host supplies `formatValue`. |
| 2026-08-11 | polish | **`dontSelect` became `openedBy: "key" \| "pointer" \| "typing"`.** One boolean was doing two unrelated jobs — `CellInput` read it as "caret at the end", `CellSelect` as "the value is the character that started the edit" — so making a click not-select would have seeded the dropdown's search box with the value the list is already scrolled to. A three-way discriminant on `CellEdit` and `EditorContext` (plus `pointerX`) has no impossible state and lets each consumer ask for what it actually cares about. Breaking, on a pre-1.0 surface whose reference page is still task 19. |
| 2026-08-11 | 19 | **The naming decision is recorded in `docs/api.md`, with the reasoning, as rule 2 of *Who this API is for* required.** `key` / `name`, no aliases: `key` is already the property name `row[column.key]` reads and the string `columnKey` names everywhere else, so calling it `field` in the column would be two words for one thing; `name` beats `title` because `title` is an HTML attribute with a different meaning. `field` / `title` are silently ignored rather than accepted — two spellings make examples drift and generated code inconsistent. The misspelling table is in the same section so a reader who arrived from the reference finds it. |
| 2026-08-11 | 19 | **The docs commit to the class names and `data-*` attributes as public, and explicitly *not* to the DOM structure.** A host needs the hooks (`data-row` / `data-col` / `data-column-key` are how a test drives the grid, and `avg-focused` and friends are how a board restyles a state), but cells are pooled and absolutely positioned and their nesting has already changed twice during the port. Saying which half is load-bearing is the only way to keep the freedom to change the other. `data-row` needed a note of its own: a header cell carries `0` and so does data row 0, and `data-type` is what tells them apart. |
| 2026-08-11 | 19 | **Two exported helper signatures were documented from the source rather than from memory, and both differed from the obvious guess.** `filterRows(rows, columns, searchString?, filters?)` takes the search *before* the filters, and `rowsToCsvText(rows, columns, withHeaders?, tabDelimiter?)` takes booleans rather than an options object. Neither is worth changing for its own sake — but a reference page that guessed them wrong would be worse than none, which is the argument for writing the docs off the code every time. |
| 2026-08-11 | 20 | **⚠ `--avg-*` set on an ancestor does nothing, and the docs said it did.** Found by running `08-theming.html` in a real browser: `:root { --avg-bg: red }` left the cells white, while `--p-bg` re-tinted everything. The cause is deliberate and correct — the grid root, `.avg-popover`, `.avg-list` and `.avg-filter-bar` each define the *whole* token block on themselves from `--p-*`, because a popover lives on `document.body` and a bar can be mounted anywhere, so none of them can rely on inheriting — but the consequence had never been written down: an element's own definition shadows any ancestor's. The rule is now three lines in `docs/api.md`, in `examples/README.md`, and at the top of the theming example: **`--p-*` on an ancestor reaches everything; `--avg-*` on an ancestor does nothing; `--avg-*` on the element itself wins.** Whether the defaults should move to `:root` so an ancestor's `--avg-*` works is a real API question and is left open — it changes the injected sheet's blast radius, so it is the user's call, not a side effect of writing examples. |
| 2026-08-11 | 20 | **Examples load `../dist/av-grid.js` and need a static server, which is stated in every file.** `dist/` is gitignored, so `npm run build` comes first; and ES module imports are blocked over `file://`, so a double-click fails with a CORS error that says nothing about the grid. The UMD build would double-click cleanly, but `.cjs` is served with a non-JS MIME type by most static servers and modern browsers refuse to execute it — so it trades one silent failure for another. One `npx serve .` in the header comment of each file is the honest cost. |
| 2026-08-11 | 20 | **Host CSS aimed at a cell has to out-specify `.avg-data-cell`.** The cell-rendering example's `.low { color: #c00 }` did nothing: the stylesheet is injected during `create()`, so it lands *after* the page's own `<style>`, and `.avg-data-cell` setting `color` is exactly as specific as a bare `.low`. `.avg-data-cell.low` settles it. Worth knowing wherever a host styles cell content — the same trap catches `onCellClass`. |
| 2026-08-11 | 20 | **The 100k example reports cells-touched-per-paint next to the flat-cost ratio, and says which one to quote.** Measured in the browser: 12.1 cells per paint at the top against 12.0 at row 99,000 — exact and identical — while the *time* ratio came back 0.83× and 0.82× on two runs, because a paint costs ~0.1 ms and timer quantization swings it either side of 1.00×. A reader who saw only the ratio would think something was wrong with a number that is actually better than the gate. Same lesson as `measureRangeDrag` in the board harness. |
