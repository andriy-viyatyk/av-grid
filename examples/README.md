...
| [`17-tree-lazy.html`](17-tree-lazy.html) | A tree whose children are fetched when a node is expanded: `treeColumn.busy` replaces the chevron with a spinner in the same slot and takes the gesture off, so pressing again during the fetch does nothing and the row never shifts. `treeColumn.spinner` swaps in the host's own |
